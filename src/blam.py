...
class PhotoModelingToolsPanel(bpy.types.Panel):
	...
	def draw(self, context):
		...
		col.label("Reconstruction:")
		col.operator("object.compute_depth_information", text="Reconstruct 3D")
		col.prop(scene, "separate_faces")
		
		col = layout.column(align=True)
		
		col.label("Projection:")
		col.operator("object.project_bg_onto_mesh", text="Project Background")
		col.row(align=True).prop(scene, "projection_method", expand=True)	   
		
		col = layout.column()
		
		col.label("3D Cursor:")
		col.operator("object.set_los_scale_pivot", text="To Camera Origin")
...
class CameraCalibrationPanel(bpy.types.Panel):
	...
	def draw(self, context): 
		...
		singleVp = scene.calibration_type == 'one_vp'
		
		layout.prop(scene, "calibration_type", text="")
		
		box = layout.box()
		split = box.row().split(percentage=0.5)
		
		split.label("Line Set 1:")
		split.row().prop(scene, "vp1_axis", expand=True)
		
		if not singleVp:
			split = box.row().split(percentage=0.5)
			
			split.label("Line Set 2:")
			split.row().prop(scene, "vp2_axis", expand=True)
			
			layout.prop(scene, "optical_center_type")
		else:
			split = box.row().split(percentage=0.5)
			
			split.label("Line Set 2:")
			split.row().prop(scene, "use_horizon_segment", text="Horizon")
			
			split = layout.row().split(percentage=0.5)
			
			split.label("Up Axis:")
			split.row().prop(scene, "up_axis", expand=True)
		...
		col.operator("clip.set_viewport_background")
...
def initSceneProps():
	...
	bpy.types.Scene.vp1_axis = bpy.props.EnumProperty(items = [('x','X','xd'),('y','Y','yd'),('z','Z','zd')],name = "Parallel to the", description=desc, default = ('x'))
	desc = "The axis to which the line segments from the second layer are parallel."
	bpy.types.Scene.vp2_axis = bpy.props.EnumProperty(items = [('x','X','xd'),('y','Y','yd'),('z','Z','zd')],name = "Parallel to the", description=desc, default = ('y'))
	desc = "The up axis for single vanishing point calibration."
	bpy.types.Scene.up_axis = bpy.props.EnumProperty(items = [('x','X','xd'),('y','Y','yd'),('z','Z','zd')],name = "Up Axis", description=desc, default = ('z'))
	...
	bpy.types.Scene.projection_method = bpy.props.EnumProperty(items = [('simple','Simple','Uses UV coordinates projected from the camera view. May give warping on large faces.'),('hq','HQ','Uses a UV Project modifier combined with a simple subdivision modifier.')],name = "Method", description=desc, default = ('hq'))
